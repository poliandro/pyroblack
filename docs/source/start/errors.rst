...
pyroblack errors all live inside the ``errors`` package:
...
The ``RPCError`` packs together all the possible errors Telegram could raise, but to make things tidier, pyroblack
...
For a fine-grained control over every single error, pyroblack does also expose errors that deal each with a specific
issue. For example:
...
In case pyroblack does not know anything about a specific error yet, it raises a generic error from its known category,
