...
At this point, we have successfully :doc:`installed pyroblack <../intro/install>` and :doc:`authorized <auth>` our
...
Making API calls with pyroblack is very simple. Here's a basic example we are going to examine step by step:
...
#.  Finally, we tell Python to schedule our ``main()`` async function by using pyroblack's :meth:`~pyrogram.Client.run`
    method.
